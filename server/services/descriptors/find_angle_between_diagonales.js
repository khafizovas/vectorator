const describeParallelogramDiagonalesAngles = ({ task, solution }) => {
	...
	describedSolution.push({
      ...
      |${solution[3].value[0]} * ${solution[1].value[0]} + 
        ${solution[3].value[1]} * ${solution[1].value[1]} + 
        ${solution[3].value[2]} * ${solution[1].value[2]}| / 
      sqrt(${solution[3].value[0]}^2 + 
        ${solution[3].value[1]}^2 + 
        ${solution[3].value[2]}^2) = 
      ${solution[4].value}`,
	});

	describedSolution.push({
		description: 'Найдём арккосинус',
		action: `alpha = acos(${solution[4].value}) = ${solution[5].value}`,
	});

	describedSolution.push({
		description: 'Переведём радианы в градусы',
		action: `alpha = ${solution[5].value} * 180 / pi = 
      ${solution[6].value}`,
	});

	describedSolution.push({
		description: 'Найдём второй угол',
		action: `beta = 180 - alpha = 180 - ${solution[6].value} = ${solution[7].value}`,
	});
